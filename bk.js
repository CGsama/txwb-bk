	...
	setTimeout(function(){
		if(state == 0){
			...
			driver.getPageSource().then(function(str){
				str.match(/>\s*(20\d+)年(\d+)月(\d+)日[0-9:\s]+<\/a>\s*<\/span>\s*<div class="funBox">/g).forEach(function(datestr){
					match = datestr.match(/>\s*(20\d+)年(\d+)月(\d+)日[0-9:\s]+<\/a>\s*<\/span>\s*<div class="funBox">/);
